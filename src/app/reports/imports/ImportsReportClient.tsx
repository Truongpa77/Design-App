...
interface ImportsReportClientProps {
  materials: Material[];
  warehouses: Warehouse[];
  customers: Customer[];
}

export default function ImportsReportClient({
  materials,
  warehouses,
  customers,
}: ImportsReportClientProps) {
  // Filters state
  ...
  const handleLoadReport = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/imports?${params.toString()}`);
      if (!res.ok) {
  ...
  const handleDetailRowDoubleClick = (item: DetailReportItem) => {
    window.open(`/vouchers/import?edit_id=${item.voucher_id}`, "_blank");
  };

  // Export current view data to Excel
  const handleExportExcel = () => {
    const title = view === "summary" ? "BÁO CÁO TỔNG HỢP NHẬP KHO" : `BÁO CÁO CHI TIẾT NHẬP KHO - ${activeMaterial?.name || ""}`;
    ...
    if (view === "summary") {
      ...
      headers = ["STT", "Số chứng từ", "Ngày chứng từ", "Người giao / Đối tác", "Kho nhập", "Số lượng", "Đơn giá (đ)", "Thành tiền trước thuế (đ)", "% Thuế", "Tiền thuế VAT (đ)", "Tổng thanh toán (đ)"];
    ...
    const fileName = view === "summary" 
      ? `Bao_Cao_Tong_Hop_Nhap_${fromDate}_${toDate}.xlsx`
      : `Bao_Cao_Chi_Tiet_Nhap_${activeMaterial?.code || activeMaterial?.id}_${fromDate}_${toDate}.xlsx`;
  ...
  const handlePrintPDF = () => {
    ...
    const title = view === "summary" ? "BÁO CÁO TỔNG HỢP NHẬP KHO" : `BÁO CÁO CHI TIẾT NHẬP KHO`;
    const filterInfo = `Từ ngày: ${new Date(fromDate).toLocaleDateString("vi-VN")} - Đến ngày: ${new Date(toDate).toLocaleDateString("vi-VN")} | Kho: ${selectedWarehouseCode || "Tất cả"} | Khách hàng: ${customers.find(c => c.id.toString() === selectedCustomerId)?.name || "Tất cả"}`;
    ...
    if (view === "summary") {
      ...
      tableHeadersHtml = `
        <tr>
          ...
          <th>Người giao / Đối tác</th>
          <th style="width: 80px; text-align: center;">Kho nhập</th>
      ...
      detailData.forEach((item, idx) => {
        ...
        tableRowsHtml += `
          <tr>
            <td style="text-align: center;">${idx + 1}</td>
            <td style="font-family: monospace; font-weight: bold; color: #2563eb;">${item.document_no}</td>
  ...
  return (
    <div className="customers-container animate-fade-in" style={{ paddingBottom: "40px" }}>
      ...
      <div className="glass-panel" style={{ padding: "20px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px", alignItems: "flex-end" }}>
              ...
              <div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "6px" }}>
                {filteredMaterialsList.map(m => (
                  <label key={m.id} style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12.5px", cursor: "pointer", padding: "4px 6px", borderRadius: "4px", transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.04)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                    ...
                    />
                    <span style={{ fontFamily: "monospace", color: "#60a5fa", fontSize: "11.5px" }}>{m.material_code || "-"}</span>
        ...
        {/* Customer Filter */}
        <div className="form-group">
          <label style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "6px" }}>Khách hàng / Nhà cung cấp</label>
          <select 
            className="input-glass" 
        ...
        {/* Warehouse Filter */}
        <div className="form-group">
          <label style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "6px" }}>Nhập về kho</label>
          <select 
            className="input-glass" 
      ...
      <div className="glass-panel" style={{ padding: "24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px", marginBottom: "20px", borderBottom: "1px solid rgba(255,255,255,0.06)", paddingBottom: "16px" }}>
          <div>
            {view === "detail" && activeMaterial ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                <span 
                  onClick={handleLoadReport} 
                  style={{ color: "#60a5fa", cursor: "pointer", fontSize: "13px", display: "flex", alignItems: "center", gap: "4px", width: "fit-content" }}
                >
                  ← Quay lại báo cáo tổng hợp
                </span>
                <h2 style={{ fontSize: "20px", fontWeight: 600 }}>Báo cáo chi tiết nhập kho vật tư</h2>
                <div style={{ fontSize: "14px", color: "#10b981", fontWeight: 500, marginTop: "2px" }}>
                  Chủng loại: <span style={{ textDecoration: "underline" }}>{activeMaterial.name}</span> {activeMaterial.code ? `(${activeMaterial.code})` : ""}
                </div>
              </div>
            ) : (
              <h2 style={{ fontSize: "20px", fontWeight: 600 }}>Báo cáo tổng hợp nhập vật tư, sản phẩm</h2>
            )}
          </div>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            ...
            <button 
              className="btn-secondary" 
              style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 16px", fontSize: "13px", borderColor: "#60a5fa", color: "#60a5fa" }}
        ...
        {/* Info Instruction */}
        <div style={{ background: "rgba(96, 165, 250, 0.08)", border: "1px dashed rgba(96,165,250,0.3)", borderRadius: "8px", padding: "10px 14px", marginBottom: "16px", fontSize: "13px", color: "#93c5fd", display: "flex", alignItems: "center", gap: "8px" }}>
          💡 <strong>Hướng dẫn nhanh:</strong> 
          {view === "summary" 
            ? "Tích đúp (double-click) vào một dòng vật tư để xem báo cáo nhập kho chi tiết của vật tư đó."
            : "Tích đúp (double-click) vào một dòng phiếu nhập để mở tab mới chỉnh sửa chứng từ."}
        </div>
              ...
              <tbody>
                {summaryData.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ fontFamily: "monospace", color: "#60a5fa", fontWeight: 600 }}>{item.material_code || "-"}</td>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <span style={{ 
                          ...
                          borderRadius: "4px",
                          fontWeight: 500,
                          background: item.material_type === "Ván" ? "rgba(59,130,246,0.15)" : item.material_type === "Nẹp chỉ" ? "rgba(249,115,22,0.15)" : item.material_type.startsWith("Phụ kiện") ? "rgba(167,139,250,0.15)" : "rgba(16,185,129,0.15)",
                          color: item.material_type === "Ván" ? "#60a5fa" : item.material_type === "Nẹp chỉ" ? "#f97316" : item.material_type.startsWith("Phụ kiện") ? "#c084fc" : "#34d399"
                        }}>
                          {item.material_type}
                        </span>
                      </td>
                      ...
                      <td style={{ textAlign: "right", fontWeight: 700, color: "#10b981", fontFamily: "monospace", fontSize: "14px" }}>{fmt(item.grand_total)}</td>
                    </tr>
                  );
                })}
                {summaryData.length > 0 && (
                  <tr style={{ background: "rgba(255,255,255,0.03)", fontWeight: "bold" }}>
                    ...
                    <td style={{ textAlign: "right", padding: "16px", color: "#10b981", fontFamily: "monospace", fontSize: "14.5px" }}>{fmt(summaryTotals.grand)}</td>
                  </tr>
                )}
                {summaryData.length === 0 && (
                  <tr>
                    <td colSpan={10} style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)" }}>Không tìm thấy dữ liệu nhập kho khớp với bộ lọc</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <table className="data-table" style={{ minWidth: "1100px" }}>
              <thead>
                <tr>
                  ...
                  <th>Người giao / Nhà cung cấp</th>
                  <th style={{ width: "100px", textAlign: "center" }}>Kho nhập</th>
                  <th style={{ width: "120px", textAlign: "right" }}>Số lượng nhập</th>
                ...
                {detailData.map((item, idx) => {
                  ...
                  return (
                    <tr 
                      ...
                      style={{ cursor: "pointer", transition: "background 0.2s" }}
                      title="Kích đúp để mở trang sửa phiếu nhập"
                    >
                      <td style={{ textAlign: "center", color: "var(--text-muted)" }}>{idx + 1}</td>
                      <td style={{ fontFamily: "monospace", color: "#60a5fa", fontWeight: 700, letterSpacing: "0.04em" }}>{item.document_no}</td>
                      ...
                      <td style={{ textAlign: "right", fontWeight: 700, color: "#10b981", fontFamily: "monospace", fontSize: "13.5px" }}>{fmt(item.total_price)}</td>
                    </tr>
                  );
                })}
                {detailData.length > 0 && (
                  <tr style={{ background: "rgba(255,255,255,0.03)", fontWeight: "bold" }}>
                    ...
                    <td style={{ textAlign: "right", padding: "16px", color: "#10b981", fontFamily: "monospace", fontSize: "14px" }}>{fmt(detailTotals.grand)}</td>
                  </tr>
                )}
                {detailData.length === 0 && (
                  <tr>
                    <td colSpan={11} style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)" }}>Không có lịch sử nhập kho của vật tư này trong khoảng thời gian đã chọn</td>
                  </tr>
                )}
              </tbody>
            </table>
