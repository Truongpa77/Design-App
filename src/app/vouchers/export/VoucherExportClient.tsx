...
export default function VoucherExportClient({
  initialData,
  materials,
  warehouses,
  customers,
  ...
  const toggleExpand = async (id: number) => {
    if (expandedVouchers[id] !== undefined) {
      ...
      try {
        const res = await fetch(`/api/vouchers/export/${id}`);
        if (res.ok) {
  ...
  const openAdd = async () => {
    ...
    setIsFormOpen(true);
    try {
      const res = await fetch('/api/vouchers/export/next-no');
      if (res.ok) {
        const data = await res.json();
      ...
      setHeaderActions(
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <button 
            ...
            style={{ width: 'auto', background: 'linear-gradient(135deg, #f59e0b, #ef4444)', border: 'none', height: '38px', display: 'flex', alignItems: 'center', gap: '6px' }} 
            onClick={openAdd}
          >
            ➕ Thêm mới
          </button>
  ...
  const openEdit = async (v: Voucher) => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/vouchers/export/${v.id}`);
  ...
  const handlePrint = async (v: Voucher) => {
    try {
      const res = await fetch(`/api/vouchers/export/${v.id}`);
      if (!res.ok) return alert("Không thể tải chi tiết phiếu xuất để in");
      const data = await res.json();
      ...
      if (printWindow) {
        ...
        printWindow.document.write(`
          <html>
            <head>
              <title>Phiếu Xuất Kho - ${data.document_no || ""}</title>
              <style>
            ...
            <body>
              <div class="actions no-print">
                <button class="btn btn-primary" onclick="window.print()">🖨️ In Phiếu Xuất (Xuất PDF)</button>
              ...
              <table class="info-table">
                ...
                <tr>
                  <td class="label">Lý do xuất kho:</td>
      ...
      console.error(err);
      alert("Lỗi hệ thống khi in phiếu xuất kho.");
    }
  };
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const url = editingId ? `/api/vouchers/export/${editingId}` : '/api/vouchers/export';
            ...
            <table className="data-table">
              ...
              <tbody>
                  ...
                  return (
                    <React.Fragment key={v.id}>
                      ...
                      >
                        ...
                        <td>
                          <span style={{ fontWeight: 700, color: '#f59e0b', fontFamily: 'monospace', letterSpacing: '0.05em' }}>
                            {v.document_no}
                          </span>
                        </td>
                        ...
                        <td style={{ textAlign: 'right', fontWeight: 700, color: '#ef4444' }}>{fmt(v.total_amount || 0)}</td>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', border: '1px solid rgba(255,255,255,0.08)' }}>
                                  <thead>
                                    <tr style={{ background: 'rgba(245, 158, 11, 0.08)' }}>
                                      <th style={{ padding: '8px 10px', textAlign: 'left', color: '#f59e0b', border: '1px solid rgba(255,255,255,0.08)', fontWeight: 600 }}>Mã SP/Vt</th>
                                  ...
                                  <tbody>
                                    {itemsList.map((item, idx) => (
                                      <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                        <td style={{ padding: '8px 10px', color: '#f59e0b', fontFamily: 'monospace', border: '1px solid rgba(255,255,255,0.06)' }}>{item.material_code || '-'}</td>
                                        ...
                                        <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 600, color: '#ef4444', border: '1px solid rgba(255,255,255,0.06)' }}>{fmt(item.total_price)}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
                ...
                {filteredVouchers.length === 0 && (
                  <tr><td colSpan={8} className="text-center py-8 text-muted">Chua co phieu xuat nao</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {isFormOpen && (
        <div className="voucher-form glass-panel">
          ...
          <div className="voucher-form-header">
            <h2 className="voucher-title" style={{ background: 'linear-gradient(135deg, #f59e0b, #ef4444)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              PHIEU XUAT KHO
            </h2>
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button className="btn-primary" style={{ width: 'auto', background: 'linear-gradient(135deg, #f59e0b, #ef4444)', border: 'none' }} onClick={handleSave} disabled={saving}>
                {saving ? 'Dang luu...' : (editingId ? 'Cap nhat' : 'Luu Phieu Xuat')}
              </button>
            </div>
          </div>
            ...
            <div className="voucher-meta-row">
              ...
              <div className="voucher-field-group" style={{ maxWidth: '200px' }}>
                ...
                <input
                  ...
                  style={{ fontFamily: 'monospace', fontWeight: 700, color: '#f59e0b', letterSpacing: '0.06em' }}
          ...
          {/* Bang chi tiet - thêm cột Tồn kho */}
            ...
            <table className="voucher-detail-table">
              <thead>
                <tr>
                  ...
                  <th style={{ width: '100px', textAlign: 'right', color: '#10b981' }}>Ton kho</th>
              ...
              <tbody>
                {items.map((item, idx) => {
                  ...
                  const qty = parseFloat(item.quantity) || 0;
                  const stockOk = !mat || qty === 0 || mat.stock_quantity >= qty;
                  return (
                    <tr key={idx} className={item.material_id ? '' : 'voucher-empty-row'}
                      style={!stockOk ? { background: 'rgba(239,68,68,0.07)' } : {}}>
                      <td style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '12px' }}>
                        {item.material_id ? idx + 1 : ''}
                      </td>
                      <td>
                        <span style={{ fontFamily: "monospace", fontWeight: 600, fontSize: "12px", color: '#f59e0b' }}>
                          {mat?.material_code || "-"}
                        </span>
                      </td>
                      <td>
                      ...
                      <td style={{ textAlign: 'right', fontWeight: 600, color: mat ? (mat.stock_quantity > 0 ? '#10b981' : '#ef4444') : 'var(--text-muted)', fontSize: '13px' }}>
                        {mat ? `${mat.stock_quantity} ${mat.unit}` : ''}
                      </td>
                      <td>
                      ...
                      <td>
                        <input type="number" min="0" className="input-glass voucher-input-num"
                          placeholder="0"
                          style={!stockOk ? { borderColor: '#ef4444', color: '#ef4444' } : {}}
