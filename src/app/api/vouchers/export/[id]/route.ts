...
// GET - Chi tiet phieu xuat kho
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const { id: idStr } = await context.params;
    const id = parseInt(idStr);
    const docRes = await pool.query(
      `SELECT d.* FROM warehouse_documents d WHERE d.id = $1 AND d.type = 'export'`,
      [id]
    );
    ...
    const detailRes = await pool.query(
      `SELECT wd.*, m.name as material_name, m.unit as material_unit, m.material_code, m.stock_quantity
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Hoan tra lai ton kho tu items cu (xuat -> cong lai)
    ...
    // Kiem tra ton kho moi truoc khi cap nhat
    for (const item of items) {
      const stockRes = await client.query(
        'SELECT stock_quantity, name FROM materials WHERE id = $1',
        [item.material_id]
      );
      if (stockRes.rows.length > 0) {
        ...
        if (stock < qty) {
          ...
          return NextResponse.json(
            { error: `Khong du ton kho: "${stockRes.rows[0].name}" (ton kho: ${stock}, xuat: ${qty})` },
            { status: 400 }
          );
        }
      }
    ...
    // Tao chi tiet moi va tru ton kho
